...
export function apply(ctx: Context): void {
  ...
  ctx.tools.register(defineTool({
    ...
    execute: args => {
      ...
      if (counts.completed !== counts.total) blockers.push('completed does not equal total')
      if (counts.errored > 0) blockers.push('errored tasks present')
      if (counts.pending > 0) blockers.push('pending tasks present')
      if (counts.timeout > 0) blockers.push('timeouts present')
      if (counts.scored !== counts.total) blockers.push('scored denominator differs from total')
